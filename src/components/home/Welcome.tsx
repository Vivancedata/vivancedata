  ...
  return (
    <section className="container mx-auto py-20 px-4 md:py-32 overflow-hidden">
      <div className="relative">
        {/* Background elements */}
        <div className="absolute top-0 right-0 w-96 h-96 bg-primary/5 rounded-full blur-3xl -z-10" aria-hidden="true" />
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-20">
          {features.map((feature, index) => (
            <div
              key={index}
              className="flex flex-col p-8 bg-card dark:bg-card/50 rounded-2xl shadow-xl border border-border hover:shadow-2xl hover:border-primary/30 transition-all duration-500 group"
            >
              <div className="rounded-2xl bg-primary/10 dark:bg-primary/20 p-4 w-fit mb-6 group-hover:bg-primary/20 dark:group-hover:bg-primary/30 transition-colors duration-300">
                {/* Icon placeholder */}
              </div>
