...
|---|---|---|---|
| Build reliability | Production build succeeds in CI and local release pipeline | `npm run build` | Pass (warning present) |
| Static quality | Lint + typecheck pass with no errors | `npm run lint && npm run type-check` | Pass |
| Unit coverage | Unit test coverage >= 85% lines/branches/functions/statements | `npm run test:unit` + coverage report | Not met |
| Integration coverage | Integration test coverage >= 85% on critical flows | `npm run test:integration` + coverage report | Not met |
| E2E baseline | Critical user journeys pass (home, contact, blog, tools) | `npm run test:e2e` | Not met |
