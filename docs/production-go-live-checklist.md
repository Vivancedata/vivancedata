...
| Unit coverage | Unit test coverage >= 95% lines/branches/functions/statements | `npm run test:unit` + coverage report | Pass |
| Integration coverage | Integration test coverage >= 95% on critical flows | `npm run test:integration` + coverage report | Pass |
...
3. Close coverage gap to >=95% (unit/integration/e2e)
...
`vitest.unit.config.ts` and `vitest.integration.config.ts` define explicit coverage include sets for critical modules. If you expand or change critical flows, update those include lists in the same pull request.
